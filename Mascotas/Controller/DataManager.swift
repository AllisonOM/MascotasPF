...
class DataManager : NSObject {
    ...
    func mascotasOtros() -> [Mascota] {
        let elQuery = Mascota.fetchRequest()    // Se crea una solicitud para obtener objetos del tipo Mascota desde Core Data
        let mExcluidas = ["Perro", "Gato", "Perico", "Canario"] // Tipos de mascotas que se quieren excluir del resultado
        let filtroOtros = NSPredicate(format: "NOT (tipo IN[c] %@)", mExcluidas)
        elQuery.predicate = filtroOtros
        do {
            let resultado = try persistentContainer.viewContext.fetch(elQuery)
            return resultado
        } catch {
            print("Error al obtener mascotas 'otros': \(error)")
            return []
        }
    }
